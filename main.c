...
#define MAX_USER 20         // nombre max d'utilisateurs dans la liste des utilisateurs connus
#define MAX_CHANNEL 10      // nombre max de channels dans la liste des channels connus
#define TTL 3               // nombre de fois que le message doit être renvoyé
#define RELAY_DELAY_MS 5000U // délai avant de renvoyer un message
#define RELAY_JOB_SLOTS 4   // nombre de messages pouvant être en attente de renvoi
...
static char** list_channel; // liste des channels connus
static Fifo* fifo_msg;  // fifo des messages déjà vus pour éviter de les traiter plusieurs fois et de les renvoyer à l'infini

char pseudo[5] = "Jaxu\0";  // pseudo du client
uint32_t msg_conter = 0;    // compteur du message envoyé

                            // définition de la structure

// définition de la structure d'une tâche de renvoi
typedef struct {
...
// définition de la structure d'un utilisateur avec son nom et son numéro de séquence du message le plus récent qu'il a envoyé
...
// fonction de comparaison de deux utilisateurs pour la liste des utilisateurs connus
...
void free_user(User* user){
    free(user->username);
                            ...
                            // fonction fournie pour la configuration
...
// envoi d'un message privé, '*' si destiné à tous
int mp_cmd(int argc, char **argv)
{
    ...
    // calcul de la taille effective du message à envoyer en concaténant les arguments de la commande
    ...
    // alloue de la mémoire pour le message à envoyer et le construit en concaténant les arguments de la commande
    ...
    // construis le message en concaténant les arguments de la commande
    size_t tmp_size = 0;
    for (int i = 2; i < argc; i++) {
    ...
    // calcule la taille totale du message et l'alloue en mémoire
    ...
    // cree le message à la forme "pseudo@target:num,ttl:message"
...
{
    // récupère le message à renvoyer et le netdev pour l'envoi
    ...
    if (netdev->driver->send(netdev, &iolist) == -ENOTSUP) {
        ...
        changeResendStat(fifo_msg, job->message, RESEND_OK);    // mets à jour le statut du message dans la FIFO pour éviter de le renvoyer à nouveau si on le reçoit à nouveau
    }

    job->busy = false;
}

// planifie le renvoi d'un message après un délai
...
void lora_mesh_renvois(char* message,int SNR){
    ...
    //Cherche le tout dans le message
    int index = 0;
    while (message[index] != ':') {//On cherche le début du numéro
        index++;
    }
    index++;
    while(message[index] != ',' ){//On cherche la fin du numéro et le début du TTL
        index++;
        if(message[index] == ':'){
            return;
        }
    ...
    index++;
    int cmp = 0;
    while(message[index + cmp] != ':'){ //on cherche la fin du TLL et le début du message
        cmp++;
    }
    // Récupère le TTL
    ...
    if(ttl > 0){    // si le TTL > 0 alors on planifie le renvoi du message en décrémentant le TTL de 1 dans le message
        //On construit un nouveau message qui a un taux de 1 de moins que le message reçu
        ttl--;
        char ttl_char[11];
        ...
        _schedule_relay_send(new_message, RELAY_DELAY_MS);// planifie le renvoi du message avec le TTL décrémenté de 1 après un délai de RELAY_DELAY_MS
    }
}

// fonction de callback pour les événements du netdev, elle traite les événements de réception de message et de fin d'émission de message
static void _event_cb(netdev_t *dev, netdev_event_t event)
{
    if (event == NETDEV_EVENT_ISR) {
        ...
        case NETDEV_EVENT_RX_COMPLETE:
            ...
            // vérifie si le message n'a pas déjà été reçu
            if(!isInFifo(fifo_msg,message)){
                pushFifo(fifo_msg, message);
                lora_mesh_renvois(message,(int)packet_info.snr);    // décide si le message doit être renvoyé
            }

            // début du traitement du message
            // crée un utilisateur et récupère son pseudo dans le message
            ...
            // vérifie si le message est un message privé ou un message de channel
            if(message[MAX_USER_NAME] == '@'){  // message pour un utilisateur
                if(message[MAX_USER_NAME + 1]=='*'){    // message pour tous les utilisateurs, on ne vérifie pas le pseudo du destinataire
                }
                else{
                    // vérifie si le message nous est destiné
                    char recv_name[MAX_USER_NAME + 1];
                    strncpy(recv_name,message + MAX_USER_NAME + 1 ,MAX_USER_NAME);
                    recv_name[MAX_USER_NAME] = '\0';
                    if(strcmp(recv_name,pseudo) != 0){  // s'il ne nous est pas destiné, on ne le traite pas
                        return;
                    } 
                }
            }else{  // message pour un channel
                // Récupère le nom du channel dans le message
                int size = index - 6;
                char name_channel[size + 1];
                strncpy(name_channel,message + 5,size);
                name_channel[size] = '\0';
                for(int i = 0;i<MAX_CHANNEL;i++){   // vérifie si on est abonné à ce channel, si on ne l'est pas, on ne traite pas le message
            ...
            free(tmp);
            int i;
            // on vérifie si l'utilisateur qui a envoyé le message est déjà dans la liste des utilisateurs connus
            if((i = findElem(list_user,user->username)) != -1){ // si on le connaît déjà, on le réinsère en tête pour qu'on enlève le plus ancien si la liste est pleine
...
//fonction pour la gestion des channels

// abonnement à un channel, on ajoute le nom du channel à la liste des channel auxquels on est abonné
int subscribe_cmd(int argc, char** argv){
    if(argc < 2){
        return 1;
    }
    //récupère le nom du channel à abonner
    char* name_channel = argv[1];
    int index = -1;
    for(int i = 0;i<MAX_CHANNEL;i++){   // vérifie si on est déjà abonné à ce channel
        if(strcmp(name_channel,list_channel[i])==0){
            printf("Already sub\n");
            return 1;
        }
    ...
    strcpy(list_channel[index],name_channel);   // insère le nom du channel dans la liste des channel auxquels on est abonné
    return 0;
}

// désabonnement d'un channel, on supprime le nom du channel de la liste des channel auxquels on est abonné
int unsubscribe_cmd(int argc, char** argv){
    if(argc < 2){
        return 1;
    }
    // récupère le nom du channel à désabonner
    char* name_channel = argv[1];
    for(int i = 0;i<MAX_CHANNEL;i++){
        if(strcmp(name_channel,list_channel[i]) == 0){
            list_channel[i][0] = '\0';  // supprime le nom du channel de la liste des channel auxquels on est abonné en mettant le premier caractère à '\0'
            return 0;
        }
    }
...
// Affiche la liste des channel auxquels on est abonné
